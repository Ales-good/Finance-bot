...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ПРОВЕРЯЕМ НОВЫЙ ЛИ ЭТО ПОЛЬЗОВАТЕЛЬ С ДИАГНОСТИКОЙ
    print("🔍 Checking if user is new...")
    is_new_user = await check_if_new_user(user.id)
    print(f"📊 User {user.id} is new: {is_new_user}")
    
    if is_new_user:
        print("🎉 Showing WELCOME message for NEW user")
    ...
    print(f"📨 Sending message to user {user.id}")
